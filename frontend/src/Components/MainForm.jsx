...
export function CombinedForm() {
  ...
  async function genrateCourse() {
    console.log("generate course");
    const api = await axios.get(
      `http://localhost:8090/api/gemini/course/user/${localStorage.getItem(
        "userId"
      )}`
    );
  ...
  async function createUser() {
    ...
    const api = await axios.post(`http://localhost:8090/api/users`, formData);
    localStorage.setItem("userId", api.data.id);
    localStorage.setItem("combinedFormData", JSON.stringify(api.data));
    console.log(api.data);
  }

  async function saveCourse(){
    console.log("save course");
    const api = await axios.post(`http://localhost:8090/api/courses/user/${localStorage.getItem("userId")}`,JSON.parse(localStorage.getItem("courses")) )
    console.log(api.data);
  }
  ...
  async function main() {
    await createUser();
    setloading("");
    const courses = await genrateCourse();
    console.log(courses);
